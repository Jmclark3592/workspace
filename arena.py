...
def comp_attack(
    char_stats,
):  # function for computer to attack. Each attack adjusts for strength factor (str1)
...
def agi_check(
    stat_c, stat_p
):  # if computer AGI is greater than player's, flag = False and comp goes first
    if stat_c > stat_p:
        return flag == False
    else:
        return flag == True
...
while True:
    ...
    if (
        npc1["alive"] == 1
    ):  # if NPC1 is alive, they check AGI to see who goes first and then battle
        ...
        if flag == False:
            ...
            if npc1["hp"] <= 0:
                ...
                npc1[
                    "alive"
                ] = 0  # if NPC1 hp goes to 0 flag sets to False and runs back loop down to NPC2
        ...
        npc2["alive"] == 1
    ):  # After NPC1 dies, player1 hp is reset and we do AGI check to see who goes first for this fight
        player1["hp"] = 40
